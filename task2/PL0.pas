...
begin {statement}
    ...
    begin
        ...
        if sym = lparen then {read后应接左括号}
        begin
            repeat {循环开始，直到读取到逗号}
                getsym;
                if sym = ident then {如果第一个sym是标识符}
                begin
                    i := position(id); {记录当前符号在符号表中的位置}
                    if i = 0 then error(11) {如果i为0, 则符号表中没有id对应的符号，报错11}
                    {若i不为0，即表示符号表中由id对应的符号，但该符号的类型不是变量，则报错12}
                    else if table[i].kind <> variable then
                    begin
                        error(12);
                        i := 0;
                    end
                    else with table[i] do {如果是变量类型, 生成一条red指令，读取数据}
                        gen(red, lev-level, adr)
                end
                else error(4); {如果左括号后跟的不是标识符,报错4}
                getsym;
            until sym <> comma
        end
        else error(40); {若read后跟的不是左括号，则报错40}

        if sym <> rparen then error(22); {若没有接右括号，则报错22}
        getsym
    end
    ...
    begin
        getsym;
        if sym = lparen then {write后应接左括号}
        begin
            repeat {循环开始，直到读取到逗号}
                getsym;
        ...
        else error(40) {若write后跟的不是左括号，则报错40}
    end;
