...
type
  ...
  whilesym, dosym, callsym, constsym, varsym, procsym );//枚举类型：nul=0,ident=1,…,即记号的类码
  alfa = packed array [1..al] of char; 
  object = (constant, variable, procedure); //object和procedure都是关键字，不能直接这样命名，实验时要修改
  
  symset = set of symbol;
  fct = (lit, opr, lod, sto, cal, int, jmp, jpc); {functions}
  instruction = packed record //压缩记录
f : fct;  {功能码}
l : 0..levmax; {相对层数}
a : 0..amax; {相对地址}
end;
